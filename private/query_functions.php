<?php
  ...
  function validate_state($state, $errors=array()) {
    ...
    } elseif (!is_valid_id($state['country_id'])) {
      $errors[] = "Country ID  must be positive integer characters integer characters greater than 0.";
    }
  ...
  function validate_territory($territory, $errors=array()) {
    ...
    } elseif (!is_valid_id($territory['position'])) {
      $errors[] = "Territory position must be integer greater than 0.";
    }

     if (is_blank($territory['state_id'])) {  
      $errors[] = "State ID name cannot be blank.";
    } elseif (!is_valid_id($territory['state_id'])) {
      $errors[] = "State ID must be positive integer characters greater than 0.";
    }
  ...
  function validate_user($user, $errors=array()) {
    ...
    } elseif (!is_valid_user_name($user['username'])) {
      $errors[] = "Username must be less than 255 characters and has valid format.";
    }
    return $errors;
  }
